...
1、 Mysql 的事务隔离级别Mysql 有四种事务隔离级别，这四种隔离级别代表当存在多个事务并发冲突时，可能出现的脏读、不可重复读、幻读的问题。
...
控制（Multiversion Concurrency Control, MVCC）是一种用于处理数据库系统中并发事务的技术。MVCC 通过保存数据的多个版本，允许读操作和写操作同时进行，从而提高系统的并发性能和数据一致性。
...
### 1. 数据库连接数优化
优化数据库连接数可以有效提高数据库的响应速度和并发处理能力：
- **连接池**：使用连接池技术，如HikariCP，可以减少连接的创建和销毁时间。
- **连接数配置**：根据数据库服务器的硬件资源（CPU、内存）合理配置最大连接数。一般建议连接池的大小为数据库最大连接数的70%-80%。

使用公式进行初步计算：
最大连接数=(可用内存−系统保留内存)/每个连接占用的内存.

查看数据库最大连接数的方法因数据库类型而异。以下是MySQL和PostgreSQL的具体步骤。

### 查看MySQL的最大连接数

#### 1. 使用SQL命令
在MySQL中，可以使用`SHOW VARIABLES`命令查看最大连接数：
...
#### 2. 使用MySQL配置文件
最大连接数也可以在MySQL的配置文件中查看和修改。配置文件通常位于`my.cnf`或`my.ini`中，可以在以下位置找到：

- Unix/Linux: `/etc/my.cnf` 或 `/etc/mysql/my.cnf`
- Windows: `C:\ProgramData\MySQL\MySQL Server 8.0\my.ini`

在配置文件中查找并修改`max_connections`参数：
...
#### 1. 使用SQL命令
在PostgreSQL中，可以使用`SHOW`命令查看最大连接数：
...
#### 2. 使用PostgreSQL配置文件
最大连接数也可以在PostgreSQL的配置文件中查看和修改。配置文件通常是`postgresql.conf`，位于数据目录中，可以在以下位置找到：

- Unix/Linux: `/etc/postgresql/<version>/main/postgresql.conf`
...
通过以上步骤，可以轻松查看MySQL和PostgreSQL的最大连接数。根据需要，可以在数据库配置文件中修改该参数，并重启数据库服务以使更改生效。在实际应用中，合理配置最大连接数需要考虑硬件资源、应用程序并发需求和数据库负载等因素。

### 2. 日志写入优化
将日志写入从随机变成顺序写可以显著提高写入性能：
- **顺序写入**：通过预写日志（WAL）机制实现顺序写入，提高磁盘写入效率。
- **批量事务持久化**：将单个事务的持久化改为批量事务的持久化，减少I/O操作次数，提高写入性能。

### 3. 缓冲区大小设置
不同类型的缓冲区大小配置对数据库性能有重要影响：
- **索引缓冲区**：增大索引缓冲区可以减少磁盘I/O，提高查询效率。
- **表缓冲区**：适当增大表缓冲区可以提高数据读取速度。
- **查询缓冲区**：增加查询缓冲区大小，尤其对复杂查询有显著提升。
- **排序缓冲区**：对于需要排序的大查询，适当增加排序缓冲区可以提高排序速度。

### 4. 数据类型选择
合理选择数据类型有助于优化存储和查询效率：
- 在数据库设计中，选择合适的数据类型对于性能和存储空间优化至关重要。`VARCHAR`、`CHAR` 和 `TEXT` 是常用的字符串数据类型，各自适用于不同的场景。以下是对这三种数据类型的详细解释和选择依据：

  ### 1. CHAR (Fixed-length)

  #### 特点
  - **固定长度**：`CHAR` 类型存储固定长度的字符串。如果存储的字符串长度小于定义的长度，MySQL 会在字符串右边填充空格以达到指定长度。
  - **性能**：由于固定长度，`CHAR` 类型在存储和检索时性能较好，因为每条记录的长度一致，便于快速定位。

  #### 使用场景
  - **定长字段**：适用于存储定长字符串，例如：国家代码（如 `USA`）、MD5 哈希值（固定32字符）。
  - **性能需求**：适用于对性能要求较高且字符串长度固定的字段。
  ...
  #### 特点
  - **可变长度**：`VARCHAR` 类型存储可变长度的字符串，不会自动填充空格。长度可以指定最大值（例如 `VARCHAR(255)` 表示最大255个字符）。
  - **性能**：由于需要额外存储字符串长度的元数据，`VARCHAR` 类型的性能略低于 `CHAR`，但由于节省了空间，整体上更为高效。

  #### 使用场景
  - **变长字段**：适用于存储长度不固定的字符串，例如：用户名、电子邮件地址、描述性字段。
  - **节省空间**：适用于需要节省存储空间的场景，因为`VARCHAR` 只存储实际使用的长度。
  ...
  #### 特点
  - **大文本数据**：`TEXT` 类型用于存储大文本数据，可以存储比 `VARCHAR` 更大的字符串（例如 `TEXT` 最大存储64KB字符，`MEDIUMTEXT` 最大16MB字符，`LONGTEXT` 最大4GB字符）。
  - **性能**：由于可以存储大量数据，`TEXT` 类型在处理大文本时性能较低。通常，`TEXT` 类型字段存储在不同的存储页中，增加了额外的 I/O 开销。

  #### 使用场景
  - **大文本字段**：适用于存储大量文本数据，例如：文章内容、评论、日志信息。
  - **非关键字段**：适用于非关键字段，因为在索引和查询操作中，`TEXT` 类型字段的性能较低。
  ...
  ```
    ### 3. 如果能满足需求,类型越简单越好整型比字符操作代价更低，因为字符集和校对规则是字符比较比整型比较更复杂
    使用mysql自建类型而不是字符串来存储日期和时间
    用整型存储IP地址(select INET_ATON(' 1.1.1.1’),select INET_NTOA(16843009))

```sql
select INET_ATON('192.168.2.1');
...
```
  ...
  根据具体使用场景选择合适的字符串数据类型可以有效优化数据库性能和存储空间。

  #### 选择依据
  - **固定长度 vs 可变长度**：如果字符串长度固定，使用`CHAR`；如果字符串长度可变，使用`VARCHAR`。
  - **数据大小**：如果数据量较小且长度变化不大，使用`VARCHAR`；如果数据量较大，使用`TEXT`。
  - **性能要求**：对于高性能需求的场景，优先选择`CHAR`或`VARCHAR`；对于需要存储大文本数据的场景，选择`TEXT`。

  以下是选择示例：

  - **国家代码**：`CHAR(3)`，因为国家代码长度固定且性能要求高。
  - **用户名**：`VARCHAR(50)`，因为用户名长度可变且通常不超过50字符。
  - **文章内容**：`TEXT`，因为文章内容长度较大且变化范围广。

  通过合理选择字符串数据类型，可以在不同场景下有效平衡性能和存储空间。

  

- 在数据库设计中，选择合适的时间类型对于性能、存储空间和功能的实现非常重要。`DATETIME`和`TIMESTAMP`是两种常用的时间类型，各自有不同的特点和适用场景。以下是对这两种时间类型的详细解释和选择依据：

  ### 1. DATETIME

  #### 特点
  - **存储格式**：`DATETIME` 类型存储的格式为`YYYY-MM-DD HH:MM:SS`，表示从1000-01-01 00:00:00到9999-12-31 23:59:59的日期和时间。
  - **存储空间**：占用8个字节的存储空间。
  - **时区独立**：`DATETIME` 类型不依赖于时区，存储的是实际的日期和时间，适用于不需要考虑时区转换的应用场景。

  #### 使用场景
  - **时区不敏感的时间**：适用于需要存储固定时间点的场景，如出生日期、事件时间等，不会因时区变化而影响。
  - **长期记录**：适用于需要存储历史记录和长期数据的场景，因为其时间范围更广，能存储到9999年。
  ...
  #### 特点
  - **存储格式**：`TIMESTAMP` 类型存储的是从1970-01-01 00:00:01 UTC到2038-01-19 03:14:07 UTC的秒数。
  - **存储空间**：占用4个字节的存储空间，比`DATETIME`更节省空间。
  - **时区相关**：`TIMESTAMP` 类型依赖于时区，存储的是UTC时间，插入和查询时会自动根据时区转换。适用于需要考虑时区的应用场景。

  #### 2038年问题
  - **2038年问题**：由于`TIMESTAMP` 类型的时间范围限制，无法表示2038-01-19 03:14:07之后的时间。解决方案包括使用`DATETIME` 类型或升级到支持更大时间范围的新系统。

  #### 使用场景
  - **时区敏感的时间**：适用于需要考虑时区转换的场景，如记录日志时间、交易时间等。
  - **当前时间**：适用于存储相对于当前时间的记录，如记录创建时间、更新时间等。
  ...
  | -------- | ------------------------------------------ | ------------------------------------------ |
  | 时间范围 | 1000-01-01 00:00:00 到 9999-12-31 23:59:59 | 1970-01-01 00:00:01 到 2038-01-19 03:14:07 |
  | 存储空间 | 8 字节                                     | 4 字节                                     |
  | 时区支持 | 不受时区影响                               | 受时区影响                                 |
  | 默认值   | 不支持自动设置当前时间                     | 支持自动设置当前时间                       |
  | 适用场景 | 存储历史数据、时区不敏感的数据             | 存储日志、时区敏感的数据                   |

  ### 选择依据

  - **时区考虑**：
    - 使用`TIMESTAMP`，如果数据需要根据时区转换（如全球用户的登录时间）。
    - 使用`DATETIME`，如果数据不需要时区转换（如出生日期）。

  - **存储空间**：
    - 使用`TIMESTAMP`，如果需要节省存储空间。
    - 使用`DATETIME`，如果存储空间不是主要考虑因素。

  - **时间范围**：
    - 使用`DATETIME`，如果需要存储2038年之后的时间。
    - 使用`TIMESTAMP`，如果时间范围在1970-2038年之间。
  ...
  通过合理选择时间类型，可以在不同的应用场景下优化性能和存储空间，同时满足功能需求。

  

### 5. 查询优化
避免不必要的全表扫描，利用索引覆盖和索引下推提高查询效率：
- **EXPLAIN**：使用EXPLAIN命令分析查询执行计划，找到性能瓶颈。
- **Optimizer Trace**：比EXPLAIN更详细，可以帮助深入分析优化器的决策过程。
- **索引覆盖**：尽量让查询使用覆盖索引，避免访问数据行，提高查询效率。
- **索引下推**：减少不必要的数据行访问，提高查询效率。
- **避免SELECT \***：只查询需要的字段，减少数据传输量。

### 6. 存储架构优化
优化存储架构可以显著提高数据库性能和可扩展性：
- **索引优化**：首先优化索引，确保查询尽量利用索引。
- **缓存**：利用缓存（如Redis、Memcached）减少数据库直接访问，提高响应速度。
- **异步化**：将部分写操作异步化，减少主库压力。
- **分库分表**：根据具体需求进行分库分表。
  - **分表**：当单表数据量过大，影响索引效率时，可以进行分表处理。
  - **分库**：当并发量过高，连接数不够用时，可以进行分库处理。
- **读写分离**：将读操作和写操作分离，读操作走从库，写操作走主库，减轻主库压力。
- **主从延迟**：考虑读写分离时，要关注主从延迟对业务的影响。
- **冷热分离**：定期对数据进行冷热分离，优化数据存储和查询效率。

### 7. 数据库反范式设计
反范式设计可以提高查询效率，但也会增加写操作复杂度和存储空间：
- **加冗余字段**：适当增加冗余字段，减少多表联查，提高查询效率。
- **分布式事务**：在有分布式事务需求时，需要权衡业务一致性和性能。

### 8. 业务需求权衡
根据业务需求权衡一致性和性能：
- **读写占比**：分析读写操作的比例，选择合适的优化策略。
- **一致性要求**：根据业务对一致性的要求，选择合适的事务处理方式。

通过以上多方面的优化，可以在不同层次上提升数据库的性能和可扩展性。优化是一个逐步的过程，需要不断监控和调整。
...
```sql
select * from 表明 limit [offset,] rows;
第一个参数:指定第一个返回记录行偏移量，注意是从0开始
第二个参数:指定返回记录行的最大数目。
如果只给定了一个参数，他表示返回最大记录行数目
```

情况1:如果偏移量是固定，返回的记录数是变化的，返回的记录数不断增加，对执行的效率有什么影响吗?
...
结论:在查询记录时，如果查询记录量相同，偏移量是不断变化的，那么超过100以后随着偏移量的不断的增加，査询的时间就会急剧增加。



优化方式1:通过索引进行分页

- 直接进行limit操作，会产生全表扫描，速度会很慢，limit限制是从结果集M位置处取出N条数据，其他抛弃。
- 假设id是连续递增，我们可以根据查询的页数和查询的记录数算出要查询的id的范围，然后配合limit使用.
...
```sql
INSERT INTO `test_dead_lock`(`id`, `NAME`, `age`) VALUES (20, 'Jim', 15);
INSERT INTO `test_dead_lock`(`id`, `NAME`, `age`) VALUES (30, 'Bob', 12);

mysql> select * from test_dead_lock;
+----+----------+------+
| id | NAME     | age  |
+----+----------+------+
|  1 | lisi     |   11 |
...
事务A执行
begin;
update test_dead_lock set age = 20 where id = 25;

mysql> select * from performance_schema.data_locks\G;
*************************** 1. row ***************************
               ENGINE: INNODB
       ENGINE_LOCK_ID: 2598714283264:1064:2598704712728
ENGINE_TRANSACTION_ID: 5921 -- 事务AID
            THREAD_ID: 48
             EVENT_ID: 31
        OBJECT_SCHEMA: study_demo
          OBJECT_NAME: test_dead_lock
       PARTITION_NAME: NULL
    SUBPARTITION_NAME: NULL
           INDEX_NAME: NULL
OBJECT_INSTANCE_BEGIN: 2598704712728
            LOCK_TYPE: TABLE -- 表锁
            LOCK_MODE: IX	 -- 意向锁
          LOCK_STATUS: GRANTED
            LOCK_DATA: NULL
*************************** 2. row ***************************
               ENGINE: INNODB
       ENGINE_LOCK_ID: 2598714283264:2:4:6:2598703333400
ENGINE_TRANSACTION_ID: 5921 -- 事务AID
            THREAD_ID: 48
             EVENT_ID: 31
        OBJECT_SCHEMA: study_demo
    ...
    SUBPARTITION_NAME: NULL
           INDEX_NAME: PRIMARY
OBJECT_INSTANCE_BEGIN: 2598703333400
            LOCK_TYPE: RECORD -- 表示行级锁
            LOCK_MODE: X,GAP  -- X类型的间隙锁
          LOCK_STATUS: GRANTED
            LOCK_DATA: 30 	  -- 锁范围最右值
2 rows in set (0.00 sec)
...
-- 通过查询锁信息，可以看出共加了两个锁：
	表锁：X类型的意向锁
	行锁：X类型的间隙锁
LOCK_TYPE中的RECORD表示行级锁，不是行级锁分类下的记录锁，可以通过LOCK_MODE类确定行级锁类型。
	LOCK MODE=X,GAP，表示是间隙锁。
	LOCK MODE=X，表示是next-key-lock临键锁。
	LOCK MODE=X,REC_NOT_GAP,表示是记录锁.

得出结论：此时事务A在主键索引上加的间隙锁，锁的范围是：(20,30)

事务B执行
begin;
update test_dead_lock set age = 20 where id = 26;

mysql> select * from performance_schema.data_locks\G;
*************************** 1. row ***************************
               ENGINE: INNODB
       ENGINE_LOCK_ID: 2598714284040:1064:2598704713496
ENGINE_TRANSACTION_ID: 5922
            THREAD_ID: 49
             EVENT_ID: 29
        OBJECT_SCHEMA: study_demo
          OBJECT_NAME: test_dead_lock
    ...
    SUBPARTITION_NAME: NULL
           INDEX_NAME: NULL
OBJECT_INSTANCE_BEGIN: 2598704713496
            LOCK_TYPE: TABLE
            LOCK_MODE: IX
          LOCK_STATUS: GRANTED
            LOCK_DATA: NULL
*************************** 2. row ***************************
               ENGINE: INNODB
       ENGINE_LOCK_ID: 2598714284040:2:4:6:2598703336472
ENGINE_TRANSACTION_ID: 5922
            THREAD_ID: 49
             EVENT_ID: 29
        OBJECT_SCHEMA: study_demo
    ...
    SUBPARTITION_NAME: NULL
           INDEX_NAME: PRIMARY
OBJECT_INSTANCE_BEGIN: 2598703336472
            LOCK_TYPE: RECORD
            LOCK_MODE: X,GAP
          LOCK_STATUS: GRANTED
            LOCK_DATA: 30
...
得出结论：此时事务B也是X型的间隙锁，锁的范围也是：(20,30)

事务A和事务B的间隙锁的范围是一样的，为什么不会冲突呢?原因就是 两个事务之间的间隙锁是相互兼容的，不会成产生冲突

事务A执行
INSERT INTO `test_dead_lock`(`id`, `NAME`, `age`) VALUES (27, 'sony', 28);
-- 事务A此时出现阻塞
...
*************************** 5. row ***************************
               ENGINE: INNODB
       ENGINE_LOCK_ID: 2598714283264:2:4:6:2598703333744
ENGINE_TRANSACTION_ID: 5921
            THREAD_ID: 48
             EVENT_ID: 33
        OBJECT_SCHEMA: study_demo
    ...
    SUBPARTITION_NAME: NULL
           INDEX_NAME: PRIMARY
OBJECT_INSTANCE_BEGIN: 2598703333744
            LOCK_TYPE: RECORD
            LOCK_MODE: X,GAP,INSERT_INTENTION
          LOCK_STATUS: WAITING -- 此时事务A处于等待状态
            LOCK_DATA: 30
5 rows in set (0.00 sec)

可以看到事务A的状态为，LOCK_STATUS:WAITING 。因为事务B生产了间隙锁,锁定的范围是(20,30)。事务A想要向该间隙插入数据，需要生成一个意向锁:LOCK MODE:INSERT_INTENTION ，插入意向锁。每插入一条新纪录，就需要看一下待插入的记录的下一条是否已经加了间隙锁，如果已经加了间隙锁，此时就会生成一个插入意向锁，然后锁的状态设置为等待状态。

INSERT INTO `test_dead_lock`(`id`, `NAME`, `age`) VALUES (28, 'sony', 23);
...
#### 为什么会发生死锁?
在当前案例中，事务A和事务B在执行完update操作后，都持有了范围是(20,30)的间隙锁，接下来两个事务都去执行插入操作，但是插入操作需要获取插入意向锁，所以都需要等待对方释放各自的间隙锁。于是就造成了循环等待状态，发生死锁。

#### 死锁总结:

- 两个事务即使生成的间隙锁的范围是一样的，也不会冲突。因为间隙锁目的是为了防止其他事务插入数据，因此间隙锁和间隙锁之间是兼容。
- 在执行插入语句的时候，如果插入的记录在其他事务持有的间隙锁的范围内，插入语句就会阻塞，因为插入语句在碰到间隙锁时，会生成一个插入意向锁，插入意向锁和间隙锁是互斥的关系。
- 满足死锁有四个条件:互斥、占有且等待、不可强占用、循环等待，就会发生死锁。


Innodb和myisam区别
InnoDB和MyISAM是MySQL中两种主要的存储引擎，它们在多个方面存在显著的区别。12

存储结构：

MyISAM将数据存储为三个文件：.frm（表定义）、.MYD（数据文件）、.MYI（索引文件）。
InnoDB由.frm文件、表空间（独立表空间或共享表空间）和日志文件（redo log）组成。
事务支持：

MyISAM不支持事务处理，每次查询具有原子性，执行速度比InnoDB快，但不提供事务支持。
InnoDB提供事务支持，具有事务（commit）、回滚（rollback）和崩溃恢复能力的事务安全（ACID compliant）型表。
锁机制：

MyISAM使用表级锁，锁定整个表，如果在锁定期间有其他进程尝试写操作，可能会导致等待，降低并发性能。
InnoDB支持行级锁定，只锁定需要修改的行，提高并发性能，但可能导致死锁。
外键支持：

MyISAM不支持外键。
InnoDB支持外键，可以通过外键约束实现关联查询和级联删除等功能。
全文索引：

MyISAM支持FULLTEXT类型的全文索引。
InnoDB不支持FULLTEXT类型的全文索引，但可以通过Sphinx插件支持全文索引。
备份和恢复：

MyISAM在跨平台的数据转移中方便，可以单独针对某个表进行备份和恢复操作。
InnoDB的备份和恢复可能相对复杂，尤其是当数据量达到几十G时。
默认存储引擎的变迁：

在MySQL 5.1之前的版本中，默认的存储引擎是MyISAM。从MySQL 5.5之后的版本中，默认的存储引擎变更为InnoDB。
性能测试：随着CPU核数的增加，InnoDB的吞吐量反而越好，而MyISAM的吞吐量几乎没有什么变化。显然，MyISAM的表锁定机制降低了读和写的吞吐量。